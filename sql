...
WHERE 
    ...
    AND EXISTS (
        SELECT 1
        FROM COMPANY c1
        JOIN EB_LOGIN_LOG_B l1 ON c1.COMPANY_KEY = l1.COMPANY_KEY
...
-- 近六個月未登入Fubon+ & 為信用卡會員客戶ID(不重複)-->可撈六個月前有登入過，但近六個月未登入的客戶ID
...
WHERE 
    ...
    AND EXISTS (
        SELECT 1
        FROM EB_LOGIN_LOG_B l
        WHERE c.COMPANY_KIND = c.COMPANY_KIND
...
1：不能勾選申辦業務子系統';
  ...
  CREATE TABLE "DBUSERNEB"."EB_LOGIN_LOG" 
